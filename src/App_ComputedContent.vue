...
<template>
  ...
  <div class="badge ml-3 bg-success">result: {{filteredProducts.length}}</div>
	<input v-model.number="max" type="range" class="form-range" min="0" max="130" />

	<div v-for="(item, index) in filteredProducts" :key="item.id" id="item-list" class="row d-flex mb-3 align-items-center">
	<div class="col-sm-4">
		<img class="img-fluid d-block" :src="item.image" :alt="item.name">
	</div>
	<div class="col">
		<h3 class="text-primary">{{index+ 1}}) {{item.name }}</h3>
